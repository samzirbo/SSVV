...
public class NotaXMLRepository extends AbstractXMLRepository<Pair<String, String>, Nota> {
    ...
    public void createFile(Nota notaObj) {
        ...
        StudentXMLRepository srepo = new StudentXMLRepository(sval, "IO/studenti.xml");
        TemaXMLRepository trepo = new TemaXMLRepository(tval, "IO/teme.xml");

        Student student = srepo.findOne(idStudent);
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("IO/" + student.getNume() + ".txt", false))) {
